...
# Source data folder
path_dataset = './datasets/LSA64_DVS_Right/'
files = os.listdir(path_dataset + 'LSAData/')
parser = aermanager.parsers.parse_dvs_128

# Target data folder
if not os.path.isdir(path_dataset + 'LSA64_DVS_Right_splits'):
    os.mkdir(path_dataset + 'LSA64_DVS_Right_splits')
    os.makedirs(path_dataset + 'LSA64_DVS_Right_splits/' + f'dataset_Right_{chunk_len_us}/train')
    # os.makedirs(path_dataset + 'LSA64_DVS_Right_splits/' + f'dataset_Right_{chunk_len_us}/validation')
    os.makedirs(path_dataset + 'LSA64_DVS_Right_splits/' + f'dataset_Right_{chunk_len_us}/test')
...
for events_file in tqdm(files):
    ...
    filename_dst = path_dataset + 'LSA64_DVS_Right_splits/' + f'dataset_{{}}_{chunk_len_us}/{{}}/' + \
    ...
    if events_file in train_samples_4sets:
        pickle.dump(total_frames, open(filename_dst.format('Right', 'train'), 'wb'))
    # if events_file in validation_samples_4sets:
    #     pickle.dump(total_frames, open(filename_dst.format('Right', 'validation'), 'wb'))
    if events_file in test_samples_4sets:
        pickle.dump(total_frames, open(filename_dst.format('Right', 'test'), 'wb'))
