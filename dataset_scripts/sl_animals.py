...
if not os.path.isdir(path_dataset + 'SL_animal_splits'):
    ...
    os.makedirs(path_dataset + 'SL_animal_splits/' + f'dataset_4sets_{chunk_len_us}/validation')
    ...
    os.makedirs(path_dataset + 'SL_animal_splits/' + f'dataset_3sets_{chunk_len_us}/validation')
    os.makedirs(path_dataset + 'SL_animal_splits/' + f'dataset_3sets_{chunk_len_us}/test')

train_samples_4sets, test_samples_4sets = train_test_split(files, test_size=train_test_size, random_state=0,
                                                           stratify=[f[:-6].split('_')[-1] for f in files])
train_samples_4sets, validation_samples_4sets = train_test_split(train_samples_4sets, test_size=train_validation_size,
                                                                random_state=0, stratify=[f[:-6].split('_')[-1] for f in train_samples_4sets])

train_samples_3sets, test_samples_3sets = train_test_split([f for f in files if '_indoor' not in f],
                                                           test_size=train_test_size, random_state=0,
                                                           stratify=[f[:-6].split('_')[-1] for f in files if
                                                                     '_indoor' not in f])
train_samples_3sets, validation_samples_3sets = train_test_split(train_samples_3sets, test_size=train_validation_size,
                                                                random_state=0, stratify=[f[:-6].split('_')[-1] for f in train_samples_3sets if '_indoor' not in f])
...
for events_file in tqdm(files):
    ...
    for _, row in labels.iterrows():
        ...
        total_events = np.array(
            [sample_events['x'], sample_events['y'], sample_events['t'], sample_events['p']]).transpose()
        ...
        if '_sunlight' in events_file:
            val_set = 'S4'  # S4 indoors with frontal sunlight
        elif '_indoor' in events_file:
            val_set = 'S3'  # S3 indoors neon light
        elif '_dc' in events_file:
            val_set = 'S2'  # S2 natural side light
        elif '_imse' in events_file:
            val_set = 'S1'  # S1 natural side light
        else:
            raise ValueError('Set not handled')

        if events_file in train_samples_3sets:
            pickle.dump(total_frames, open(filename_dst.format('3sets', 'train', val_set, row['class']), 'wb'))
        if events_file in validation_samples_3sets:
            pickle.dump(total_frames, open(filename_dst.format('3sets', 'validation', val_set, row['class']), 'wb'))
        ...
        if events_file in validation_samples_4sets:
            pickle.dump(total_frames, open(filename_dst.format('4sets', 'validation', val_set, row['class']), 'wb'))
